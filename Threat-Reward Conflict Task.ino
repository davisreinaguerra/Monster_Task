...
2. Pin Number Assignments and Variable Initialization
___________________________________________________________________________*/
...
#define monster_sound = "/godzilla.wav"
#define pink_sound = "/pink.wav"
#define no_sound = "/.wav"
#define experiment_sound = ""
...
void sound_filename_asker() // function g
{
...
void establish_config() // function i
{
    ...
    Serial.println("The trial number is: "); Serial.print(trial_number_asker());
    Serial.println("The intertrial interval is: "); Serial.print(intertrial_interval_asker());
    if (monster_trial == true) 
      {
      Serial.println("This is a monster trial");
      } else {}
    if (monster_trial == false)
      {
      Serial.println("This is not a monster trial");
      } else {}
    Serial.println("The experiment filename is: "); Serial.print(sound_filename_asker());
    ...
    switch (c) 
    {
      case 'y': // here, you want to define all of the variables as the output of the function only if the configuration is correct
        ...
        experiment_sound = experiment_filename_asker();
...
void run_trials() // have to add escape routes for if trial fails
{
  ...
  for (int i = 1; i < trial_number; i++) {
    ...
    while (wait_for_return()); // wait until mouse in door
...
int wait_for_return()
{
  while (MOUSE_IN_DOOR_PIN == HIGH) {return 0};
