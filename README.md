...
### Trial Configuration
Each trial is setup via a MATLAB App which collects trial variables and writes the value of those variables to the serial port in sequential order.  Pressing the start button on the MATLAB GUI runs the below Serial port write function.  For MATLAB configuration, [click here]()
